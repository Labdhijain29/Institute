...
export function MarketingPortal({ path }) {
  const { user, logout } = useAuth(); const sidebarStorageKey = "crm_sidebar_collapsed_DigitalMarketingExecutive"; const [open, setOpen] = useState(() => window.innerWidth >= 1024 && localStorage.getItem(sidebarStorageKey) !== "true");
  const active = menu.find((item) => item[1] === path) || menu[0];
  const signOut = () => { logout(); go("/login", true); };
  useEffect(() => { if (window.innerWidth >= 1024) localStorage.setItem(sidebarStorageKey, String(!open)); }, [open]);
  const closeMobileMenu = () => { if (window.innerWidth < 1024) setOpen(false); };
  return <div className={`min-h-screen bg-[#f8f5ef] text-[#111315] lg:grid ${open ? "lg:grid-cols-[270px_minmax(0,1fr)]" : "lg:grid-cols-[0_minmax(0,1fr)]"}`}>
    {open && <button className="fixed inset-0 z-20 bg-black/30 lg:hidden" onClick={() => setOpen(false)} />}
    <aside className={`fixed inset-y-0 left-0 z-30 w-[270px] overflow-hidden border-r border-slate-200 bg-white transition-[transform,width] duration-300 ease-in-out lg:static ${open ? "translate-x-0 lg:translate-x-0 lg:w-[270px]" : "-translate-x-full lg:w-0"}`}>
