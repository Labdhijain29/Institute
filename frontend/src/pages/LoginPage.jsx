...
export function LoginPage({ initialMode = "login" }) {
  ...
  return (
    <main className="grid min-h-screen grid-cols-1 bg-[#f6f8f7] text-ink lg:grid-cols-[1.05fr_0.95fr]">
      <section className="relative flex min-h-[34vh] items-end overflow-hidden bg-ink px-8 py-10 text-white lg:min-h-screen lg:px-14">
        <img
          src="https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=1400&q=80"
          alt=""
          className="absolute inset-0 h-full w-full object-cover opacity-45"
        />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(23,32,38,.88),rgba(15,118,110,.55))]" />
        <div className="relative max-w-2xl">
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-amberline">Institute CRM ERP</p>
      ...
      <section className="flex items-center justify-center px-5 py-6">
        ...
        <div className="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-soft">
          ...
          <form onSubmit={submitLogin} className="mt-9 space-y-5">
            ...
            <button className="flex h-14 w-full items-center justify-center gap-2 rounded-md bg-ink font-bold text-white hover:bg-pine">
              <LogIn size={18} />
              Login
            </button>
          </form>
          ...
          <p className="mt-6 text-center text-sm text-slate-500">
            ...
            <button className="font-semibold text-pine" onClick={() => { setMode("register"); setError(""); setSuccess(""); }}>
              Register
            </button>
          </p>
        </div>
        ...
        <div className="w-full max-w-[660px] rounded-lg border border-slate-200 bg-white px-5 py-4 shadow-soft md:px-7">
          ...
          <form onSubmit={submitRegister} className="mt-5 grid gap-x-4 gap-y-3 md:grid-cols-2">
            ...
            <button className="h-11 rounded-md bg-ink text-sm font-bold normal-case tracking-normal text-white hover:bg-pine md:col-span-2">Register</button>
          </form>

          <p className="mt-3 text-center text-sm text-slate-500">
            Already have an account?{" "}
            <button className="font-semibold text-pine" onClick={() => { setMode("login"); setError(""); }}>
              Login
            </button>
          </p>
        </div>
